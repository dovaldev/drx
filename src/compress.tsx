...
export function compressTsx(
  ...
  function flushRawGroup() {
    ...
    const formatted = code.replace(/^( {4})+/gm, (match) =>
...
function compressImport(
  ...
  if (clause.namedBindings && ts.isNamedImports(clause.namedBindings)) {
    ...
    const names = clause.namedBindings.elements.map(
      (element) =>
        reverseLookup(config.namedAliases, element.name.text) ??
        element.name.text,
    )
...
function compressSimpleStatement(
  ...
  if (
    ...
    if (declaration.initializer) {
      const initializer = printer.printNode(
        ts.EmitHint.Unspecified,
  ...
  if (ts.isReturnStatement(statement) && statement.expression) {
    const expression = printer.printNode(
      ts.EmitHint.Unspecified,
      statement.expression,
      sourceFile,
    )
...
function compressJsxNode(node: ts.Node, sourceFile: ts.SourceFile) {
...
function compressAttributes(
  ...
  for (const prop of attributes.properties) {
    ...
    if (ts.isJsxExpression(value) && value.expression) {
      parts.push(
        `${propName}=${formatCompressedExpression(value.expression.getText(sourceFile))}`,
      )
      continue
    }
...
function returnsJsx(node: ts.Node) {
