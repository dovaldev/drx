...
function compressionReportMarkdown(
  ...
  return `# DRX Compression Report Generated: ${new Date().toISOString()} ## Summary - Runtime source: \`${config.outDir}\` - DRX source: \`${config.sourceDir}\` - Files compressed: ${files.length} - Estimated original context: ${formatNumber(totals.sourceTokens)} tokens - Estimated DRX context: ${formatNumber(totals.drxTokens)} tokens - Estimated tokens saved: ${formatNumber(totals.savedTokens)} (${tokenReduction}) - Character reduction: ${formatNumber(totals.sourceChars)} -> ${formatNumber(totals.drxChars)} (${charReduction}) - Raw fallback blocks: ${formatNumber(totals.rawBlocks)} Token estimates use a simple \`characters / 4\` heuristic. Actual savings vary by model tokenizer. ## Files | Runtime | DRX | Original tokens | DRX tokens | Saved | Reduction | Raw blocks | | --- | --- | ---: | ---: | ---: | ---: | ---: | ${files.map(fileReportRow).join("\n") || "| None | None | 0 | 0 | 0 | 0% | 0 |"} `
}
