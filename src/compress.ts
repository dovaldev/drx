...
export function compressTsx(source: string, config: DrxConfig, options: CompressOptions = {}) {
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (ts.isImportDeclaration(statement)) {
      lines.push(...compressImport(statement, config))
      continue
    }
    ...
    lines.push(...rawBlock(statement.getText(sourceFile), 0))
  }
...
function compressImport(statement: ts.ImportDeclaration, config: DrxConfig) {
  ...
  if (clause.isTypeOnly) return [`raw\n  ${statement.getText()}`]

  const lines: string[] = []
  if (clause.name) lines.push(`i ${clause.name.text} f ${sourceAlias}`)

  if (clause.namedBindings && ts.isNamedImports(clause.namedBindings)) {
    if (clause.namedBindings.elements.some((element) => element.isTypeOnly)) return [`raw\n  ${statement.getText()}`]
...
function compressFunction(fn: ts.FunctionDeclaration, sourceFile: ts.SourceFile) {
  if (!fn.name) return rawBlock(fn.getText(sourceFile), 0)
  ...
  const params = fn.parameters.map((param) => param.getText(sourceFile)).join(", ")
...
function compressArrowComponent(statement: ts.VariableStatement, sourceFile: ts.SourceFile) {
  ...
  const params = initializer.parameters.map((param) => param.getText(sourceFile)).join(", ")
...
function compressUseState(statement: ts.Statement, sourceFile: ts.SourceFile) {
  ...
  const initial = declaration.initializer.arguments[0]?.getText(sourceFile) ?? "undefined"
...
function compressUseEffect(statement: ts.Statement, sourceFile: ts.SourceFile) {
  ...
  const deps = expr.arguments[1]?.getText(sourceFile) ?? "[]"
...
function rawBlock(code: string, level: number) {
  return ["raw", ...indentLines(code.split("\n"), level + 1)]
}
