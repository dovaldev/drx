...
function contextMarkdown(summary: ProjectSummary, config: DrxConfig) {
  return `# DRX AI Context Generated: ${summary.generatedAt} ## Project - DRX source: \`${summary.sourceDir}\` - Generated/runtime source: \`${summary.outDir}\` - Framework: \`${config.framework}\` - Ignore patterns: ${config.ignore.map((item) => `\`${item}\``).join(", ")} ## How To Work 1. Read \`.drx/rules.md\`. 2. Prefer editing \`${summary.sourceDir}\` when the feature touches React/Next UI. 3. Run \`pnpm dev -- check\`. 4. Run \`pnpm dev -- expand\`. 5. Do not edit generated TSX directly unless explicitly requested. ## Routes ${list(summary.routes)} ## Components ${list(summary.components)} ## Content Files ${list(summary.contentFiles)} ## Config Files ${list(summary.configFiles)} ## DRX Symbols ${symbolList(summary.symbols)} ## API / Backend Files ${list(summary.apiFiles)} ## Aliases ### Imports ${aliasList(config.aliases)} ### Tags ${aliasList(config.tagAliases)} ### Events ${aliasList(config.eventAliases)} `
}

function rulesMarkdown(config: DrxConfig) {
  return `# DRX Rules DRX is a compact JSX-compatible syntax that compiles to React/Next TSX. Core rules: - \`i X f pkg\` = \`import X from "pkg"\` - \`i {A,B} f pkg\` = \`import { A, B } from "pkg"\` - \`ed fn Name()\` = \`export default function Name()\` - \`ex fn Name()\` = \`export function Name()\` - \`fn Name()\` = \`function Name()\` - \`c name = value\` = \`const name = value\` - \`l name = value\` = \`let name = value\` - \`await\` = \`await\` - \`r value\` = \`return value\` - \`st name = value\` = \`const [name, setName] = useState(value)\` - \`ef [deps]\` = \`useEffect(() => {}, [deps])\` - \`ui\` starts the returned JSX block - \`.class\` inside a tag becomes \`className\` - \`raw\` preserves full JS/TS/TSX Prefer readable names. Use \`raw\` for unsupported syntax. Known tag aliases: ${aliasList(config.tagAliases)} `
}
